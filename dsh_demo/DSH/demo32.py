...
#    d_vars = [var for var in t_vars if 'd_' in var.name]
    ...
    optimizer = tf.train.AdamOptimizer(LR, beta1=0.5) \
        .minimize(d_loss_real,  global_step=global_step)
...
#    optimizer = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=0.9).minimize(d_loss_real,
#                                                                                               global_step=global_step)
    ...
    for epoch in range(start, EPOCH):
        ...
        for idx in range(start, batch_idxs):
            ...
            sess.run([optimizer], feed_dict={image: image_idx, label: label_idx})
